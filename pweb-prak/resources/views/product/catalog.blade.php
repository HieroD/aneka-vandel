...
<section class="catalog">
  ...
  <div class="pagination">
    <button onclick="filterCards('all')" class="{{ $kategori === 'all' ? 'active-filter' : '' }}"
      data-filter="all">All</button>
    <button onclick="filterCards('vandel')" class="{{ $kategori === 'vandel' ? 'active-filter' : '' }}"
      data-filter="vandel">Vandel</button>
    <button onclick="filterCards('prasasti')" class="{{ $kategori === 'prasasti' ? 'active-filter' : '' }}"
      data-filter="prasasti">Prasasti</button>
    <button onclick="filterCards('kijangan')" class="{{ $kategori === 'kijangan' ? 'active-filter' : '' }}"
      data-filter="kijangan">Kijangan</button>
  </div>

  {{-- SEARCH --}}
  <div class="search">
    <form action="{{ route('catalog.index', $kategori) }}" id="search-bar" method="GET">
      <input type="text" name="search" id="search-text"
        placeholder="Search..."
        value="{{ request('search') }}"
        oninput="handleSearch(this.value)" />
    </form>
    <span id="product-count">Menampilkan {{ $produks->count() }} Produk</span>
  </div>
