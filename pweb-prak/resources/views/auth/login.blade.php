  ...
  <div class="bg-[#163070] border border-dashed border-white/25 rounded-2xl p-10 w-full max-w-sm animate-[fadeUp_.45s_ease]">
    ...
    <form action="{{ route('login.store') }}" method="POST" id="loginForm">
      ...
      <div class="mb-4">
        ...
        @else
        <div id="emailErr" class="text-red-300 text-[11px] mt-1 hidden">Format email tidak valid</div>
        @enderror
      </div>
      ...
      <div class="mb-4">
        ...
        <div class="text-red-300 text-[11px] mt-1">{{ $message }}</div>
        @else
        <div id="passErr" class="text-red-300 text-[11px] mt-1 hidden">Password tidak boleh kosong</div>
        @enderror
      </div>
