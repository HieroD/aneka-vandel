<x-auth-layout title="Verify Email">
    ...
    <h1 class="text-white text-[22px] font-bold text-center mb-3">Verify your email</h1>

    <p class="text-white/60 text-[13.5px] leading-[1.7] text-center mb-2">
        Kami telah mengirimkan link verifikasi ke email Anda. Silakan klik link tersebut untuk mengaktifkan akun.
    </p>

    <div class="inline-block bg-white/10 text-white/90 text-[13px] font-semibold px-4 py-1.5 rounded-lg mb-7 break-all w-full text-center">
    ...
    <form action="{{ route('verification.send') }}" method="POST" id="resendForm">
        @csrf
        <button type="submit" id="resendBtn"
            class="w-full py-3 bg-white/95 text-primary-card font-bold text-[13px] tracking-[2px] uppercase rounded-[10px] transition hover:bg-white hover:-translate-y-[2px] shadow-lg mb-3 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">
            Resend Email
        </button>
    </form>
    ...
    <form action="{{ route('logout') }}" method="POST">
        ...
        @method('DELETE')
        <button type="submit"
            class="w-full py-3 bg-transparent border border-white/15 text-white/60 text-[13px] flex items-center justify-center gap-2 rounded-[10px] cursor-pointer">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
    ...
    <script>
        ...
        function showToast(msg) {
            ...
            setTimeout(function () { t.classList.add('-translate-y-16'); }, 3000);
        }
