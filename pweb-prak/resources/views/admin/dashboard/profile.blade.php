<x-admin-layout title="Profil Admin">
    ...
    <div class="grid grid-cols-[1fr_260px] gap-10 items-start">
        ...
        <form method="POST" action="{{ route('admin.profile.update') }}">
            ...
            <div class="mb-5">
                ...
                <input
                    type="text"
                    name="name"
                    value="{{ old('name', auth()->user()->name ?? 'Admin') }}"
        ...
        {{-- FORM FOTO (independen, bukan bersarang di form profil) --}}
        <div class="text-center">
            ...
            <form method="post" enctype="multipart/form-data" action="{{ route('admin.avatar.update') }}">
                @csrf
                <input type="file" id="fotoInput" name="avatar" accept="image/*"
                    class="hidden"
                    onchange="this.form.submit()" />
                <x-button type="button" variant="secondary" size="md" onclick="document.getElementById('fotoInput').click()" class="cursor-pointer">
                    Pilih Foto
                </x-button>
                @error('avatar')
                <p class="text-[11px] text-danger mt-2">{{ $message }}</p>
                @enderror
            </form>
