<x-admin-layout title="Tambah Produk">
    ...
    <form action="{{ route('catalog.store') }}" method="POST" enctype="multipart/form-data" id="productForm">
        @csrf

        {{-- HEADER --}}
        <div class="flex items-start justify-between mb-7">
            <div>
                <h1 class="text-[22px] font-bold text-text mb-1">Tambah Produk</h1>
                <p class="text-[13px] text-text-subtle">Silahkan isi detail produk vandel baru Anda.</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6 items-start">
            ...
            <div class="bg-white rounded-xl border border-border p-6 flex flex-col gap-5">
                ...
                <div>
                    ...
                    @php $selectedCategory = old('category'); @endphp
        ...
        {{-- stok belum ditampilkan di form; disimpan dengan nilai default --}}
        <input type="hidden" name="total_product" value="0">
    ...
    <script>
        document.querySelectorAll('[data-category-picker]').forEach(function(picker) {
            var hidden = picker.closest('div').parentElement.querySelector('#category') || document.getElementById('category');
